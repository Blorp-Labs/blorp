...
function commentView(id: number, path: string, childCount = 0) {
  if (!path.endsWith(String(id))) {
    throw new Error("invalid path id combo");
  }
...
describe("buildCommentTree", () => {
  describe("deduplication", () => {
    test("keeps the first occurrence when duplicate comment ids are present", () => {
      const tree = buildCommentTree(
        [commentView(1, "0.1", 0), commentView(1, "0.1", 99)],
        {},
      );

      expect(getNodeByKey(tree, 1).comment?.childCount).toBe(0);
      expect(_.keys(tree.children)).toHaveLength(1);
    });
  });
  ...
  describe("tree construction", () => {
    ...
    test("preserves insertion order after deduplication", () => {
      const tree = buildCommentTree(
        [
          commentView(30, "0.30"),
          commentView(10, "0.10"),
    ...
    test("hydrates a top-level placeholder and preserves relative order of unrelated siblings", () => {
      ...
      expect(getNodeByKey(tree, 3).meta.sort).toBeGreaterThan(
        getNodeByKey(tree, 2).meta.sort,
      );
    });
  ...
  describe("path filtering", () => {
    ...
    test("re-roots matching deep paths so the requested subtree appears at top-level", () => {
      ...
      const node7890 = getNodeByKey(tree, 7890);
      expect(node7890.comment?.id).toBe(7890);
      expect(getNodeByKey(node7890, 11111).comment?.id).toBe(11111);
    ...
    test("sorts highlightCommentId to top among siblings", () => {
      const tree = buildCommentTree(
        ...
        { threadRootId: 1, selectedCommentId: 3 },
      );

      const parentNode = getNodeByKey(tree, 1);
      const node2 = getNodeByKey(parentNode, 2);
      const node3 = getNodeByKey(parentNode, 3);
      const node4 = getNodeByKey(parentNode, 4);
      expect(node3.meta.sort).toBeLessThan(node2.meta.sort);
      expect(node3.meta.sort).toBeLessThan(node4.meta.sort);
    });
  });
  ...
  describe("pruning by page cursor", () => {
    ...
    test("removes direct children loaded from a different page", () => {
      ...
      expect(getNodeByKey(tree, 1).children).toEqual({});
    });
    ...
    test("prunes a cross-page direct child even when a placeholder sibling is present", () => {
      ...
      const node1 = getNodeByKey(tree, 1);
      expect(node1.meta.pruned).toBe(true);
      expect(getNodeByKey(node1, 2)).not.toBeUndefined();
...
describe("shouldShowMore", () => {
  test("reports a hidden sibling correctly when grandchildren inflate parent childCount", () => {
    // node1 has 3 direct children, only 2 visible — should show more.
    const tree = buildCommentTree(
      [
        commentView(1, "0.1", 4),
        commentView(2, "0.1.2", 1),
  ...
  test("returns true when visible children are fewer than immediateChildren", () => {
    const node = {
      meta: {
        sort: 0,
        immediateChildren: 1,
  ...
  test("returns false when not pruned and no immediate children are missing", () => {
    const tree = {
      meta: {
        sort: 0,
        immediateChildren: 0,
...
describe("getCommentChildren", () => {
  ...
  test("returns placeholder children along with real comment children", () => {
    ...
    expect(Number(key)).toBe(5678);
    expect(child.comment).toBeUndefined();
  });
...
describe("colorIndex", () => {
  ...
  test("nested nodes increment colorIndex by 1 per level", () => {
    ...
    const node2 = getNodeByKey(node1, 2);
    const node3 = getNodeByKey(node2, 3);
    expect(node1.meta.colorIndex).toBe(0);
    expect(node2.meta.colorIndex).toBe(1);
    expect(node3.meta.colorIndex).toBe(2);
  });

  test("placeholder chain gets consecutive colorIndex values", () => {
  ...
  test("colorIndexOffset with threadRootId anchors thread colors to absolute depth", () => {
    // thread root (node 2) sits at absolute depth 2 in the full tree;
    // caller passes offset=2 to preserve the colors it would have at that depth
    const tree = buildCommentTree(
      [
        commentView(1, "0.1"),
        commentView(2, "0.1.2"),
      ...
      { threadRootId: 2, colorIndexOffset: 2 },
    );
    const node2 = getNodeByKey(tree, 2);
    const node3 = getNodeByKey(node2, 3);
    const node4 = getNodeByKey(node3, 4);
    expect(node2.meta.colorIndex).toBe(2);
    expect(node3.meta.colorIndex).toBe(3);
    expect(node4.meta.colorIndex).toBe(4);
  });
});
